...
```{r}
# Calculate breaks every 4 years
start_year <- year(min(presidential$start))
end_year <- year(max(presidential$end))
year_breaks <- seq(from = start_year, to = end_year, by = 4)
...
presidential |>
  ...
  labs(title = "Presidential Terms in the United States",
       ...
       y = "President #")
  # Placing breaks every 4 years (this is trickier than it seems!).
  # scale_x_continuous(limits = c(start_year, end_year), n.break = (end_year - start_year) / 4
...
cancelled_flights <- filter(flights, is.na(dep_time)) |>
  ...
  summarise(cancelled_flight_num = n())

total_flight <- flights |>
group_by(year, month, day) |>
  summarise(total_flight_num = n())

averaged_delay <- filter(flights, !is.na(dep_time)) |>
  group_by(year, month, day) |>
  summarise(avg_delay = mean(dep_delay))

joined_flights <- inner_join(cancelled_flights, total_flight, by=c("year", "month", "day"))

joined_flights <- inner_join(joined_flights, averaged_delay, by=c("year", "month", "day"))

joined_flights <- mutate(joined_flights, cancelled_proportions = cancelled_flights / total_flight_num)
...
**Answer: Filter for rows that have a missing value with `filter()` and then**
...
**Answer: I want to handle ties like done in min_rank() where we rank all the ties to have the same (smallest) rank. **
