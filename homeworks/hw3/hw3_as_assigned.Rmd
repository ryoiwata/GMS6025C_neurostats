...
```{r loading in data}
# Reading in the data
raw_gad65_data <- readxl::read_excel("./data/HW3data_.xlsx")
```
```{r data carpentry}
# Turning categorical variables into factors
fixed_gad65_data <- mutate(raw_gad65_data,
across(c(WellID, `Plate ID`, `Enzyme Variant`),
as_factor))
```
...
```{r}
distinct(fixed_gad65_data, `Enzyme Variant`)
```
...
```{r}
fixed_gad65_data |> ggplot(aes(x=`Enzyme Variant`, y=`Enz Activity`, color = `Enzyme Variant`)) +
  geom_violin() +
...
car_variant_to_activity_model = car::Anova(variant_to_activity_model)

car_variant_to_activity_model |> 
  select("Pr(>F)") #|>
  # filter(row.names(car_variant_to_activity_model) == "Enzyme Variant") 
# summary(variant_to_activity_model)
```
...
kable(variant_and_glutamate_to_activity_tidy)
```
...
```{r}
variant_and_pyridoxal_to_activity_model = lm(`Enz Activity` ~ `Enzyme Variant` + `pyridoxal phosphate conc.`, fixed_gad65_data)

variant_and_pyridoxal_to_activity_tidy = tidy(variant_and_pyridoxal_to_activity_model)

kable(variant_and_pyridoxal_to_activity_tidy)
...
variant_and_glutamate_to_activity_augment <- augment(variant_and_glutamate_to_activity_model) |>
  mutate(`Corrected Enz Activity` = `Enz Activity` - Glu * glu_coef_for_variant_and_glutamate_to_activity)


```

```{r model D}
augment(variant_and_pyridoxal_to_activity_model)
```
