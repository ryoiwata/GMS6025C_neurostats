...
```{r}
variant_to_activity_car
```

```{r}
variant_to_activity_pvalue <- pull(filter(variant_to_activity_car, row.names(variant_to_activity_car)=="`Enzyme Variant`"), 
                       "Pr(>F)")

variant_to_activity_fstat <- pull(filter(variant_to_activity_car, row.names(variant_to_activity_car)=="`Enzyme Variant`"), 
                       "F value")

variant_to_activity_Df <- pull(filter(variant_to_activity_car, row.names(variant_to_activity_car)=="`Enzyme Variant`"), 
                       "Df")


print("F-statistic and associated p-value for all slope terms")
print(paste("F(", variant_to_activity_Df, ",", df.residual(variant_to_activity_model), ") = ", 
            variant_to_activity_fstat, ", p = ", 
            variant_to_activity_pvalue))
```

**Assuming significance with a p-value less than 0.05, we can not reject the null hypothesis that the mean enzyme activity of different enzyme variants are not significantly different from each other**
...
**Answer: There is a significant difference in the enzyme activity in at least one of the tested GAD65 enzyme variants compared to the other tested GAD65 enzyme variants after controlling for the effect of the concentrations of glutamate.**
...
fixed_gad65_data |> ggplot(aes(x=Glu, y=`Enz Activity`, color=`Enzyme Variant`)) +
  geom_point() +
  facet_wrap(vars(`Enzyme Variant`)) +
...
```{r}
variant_and_glutamate_to_activity_model = lm(`Enz Activity` ~ `Enzyme Variant` + Glu, fixed_gad65_data)

variant_and_glutamate_to_activity_car = car::Anova(variant_and_glutamate_to_activity_model)
variant_and_glutamate_to_activity_tidy = tidy(variant_and_glutamate_to_activity_model)

kable(variant_and_glutamate_to_activity_car)
```
```{r}
variant_and_glutamate_pvalue_for_variant <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="`Enzyme Variant`"), "Pr(>F)")

variant_and_glutamate_fstat_for_variant <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="`Enzyme Variant`"), 
                       "F value")

variant_and_glutamate_df_for_variant <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="`Enzyme Variant`"), 
                       "Df")


print("F-statistic and associated p-value for all slope terms")
print(paste("F(", variant_and_glutamate_df_for_variant, ",", df.residual(variant_and_glutamate_to_activity_model), ") = ", 
            variant_and_glutamate_fstat_for_variant, ", p = ", 
            variant_and_glutamate_pvalue_for_variant))
```
```{r}
variant_and_glutamate_pvalue_for_glu <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="Glu"), "Pr(>F)")

variant_and_glutamate_fstat_for_variant <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="Glu"), 
                       "F value")

variant_and_glutamate_df_for_variant <- pull(filter(variant_and_glutamate_to_activity_car, row.names(variant_and_glutamate_to_activity_car)=="Glu"), 
                       "Df")
...
print(paste("F(", variant_and_glutamate_df_for_variant, ",", df.residual(variant_and_glutamate_to_activity_model), ") = ", 
            variant_and_glutamate_fstat_for_variant, ", p = ", 
            variant_and_glutamate_pvalue_for_glu))
```
...
```{r}
variant_and_phosphate_to_activity_model = lm(`Enz Activity` ~ `Enzyme Variant` + `pyridoxal phosphate conc.`, fixed_gad65_data)

variant_and_phosphate_to_activity_car = car::Anova(variant_and_phosphate_to_activity_model)
variant_and_phosphate_to_activity_tidy = tidy(variant_and_phosphate_to_activity_model)

kable(variant_and_phosphate_to_activity_car)
```
...
```{r}
variant_and_phosphate_pvalue_for_variant <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`Enzyme Variant`"), "Pr(>F)")

variant_and_phosphate_fstat_for_variant <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`Enzyme Variant`"), 
                       "F value")

variant_and_phosphate_df_for_variant <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`Enzyme Variant`"), 
                       "Df")


print("F-statistic and associated p-value for all slope terms")
print(paste("F(", variant_and_phosphate_df_for_variant, ",", df.residual(variant_and_phosphate_to_activity_model), ") = ", 
            variant_and_phosphate_fstat_for_variant, ", p = ", 
            variant_and_phosphate_pvalue_for_variant))
```
```{r}
variant_and_phosphate_pvalue_for_glu <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`pyridoxal phosphate conc.`"), "Pr(>F)")

variant_and_phosphate_fstat_for_variant <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`pyridoxal phosphate conc.`"), 
                       "F value")

variant_and_phosphate_df_for_variant <- pull(filter(variant_and_phosphate_to_activity_car, row.names(variant_and_phosphate_to_activity_car)=="`pyridoxal phosphate conc.`"), 
                       "Df")
...
print(paste("F(", variant_and_phosphate_df_for_variant, ",", df.residual(variant_and_phosphate_to_activity_model), ") = ", 
            variant_and_phosphate_fstat_for_variant, ", p = ", 
            variant_and_phosphate_pvalue_for_glu))
```
...
```{r}
variant_and_glutamate_to_activity_tidy
```


```{r model C}

glu_coef_for_variant_and_glutamate_to_activity <- pull(filter(variant_and_glutamate_to_activity_tidy, term == "Glu"), estimate)

phosphate_coef_for_variant_and_glutamate_to_activity <- pull(filter(variant_and_phosphate_to_activity_tidy, term == "`pyridoxal phosphate conc.`"), estimate)

glutamate_and_phosphated_corrected_gad65_data <- fixed_gad65_data |>
  mutate(`glu corrected Enz Activity` = `Enz Activity` - Glu * glu_coef_for_variant_and_glutamate_to_activity) |>
  mutate(`phosphate corrected Enz Activity` = `Enz Activity` - `pyridoxal phosphate conc.`
 * phosphate_coef_for_variant_and_glutamate_to_activity) |>
  pivot_longer(cols=c(`Enz Activity`, `glu corrected Enz Activity`, `phosphate corrected Enz Activity`), values_to = "Enz Activity")

```

```{r}
glutamate_and_phosphated_corrected_gad65_data |> ggplot(aes(x=`Enzyme Variant`, 
                               y=`Enz Activity`, 
                               color = `Enzyme Variant`)) +
  geom_violin() +
  geom_jitter(width = 0.25) +
...
glutamate_and_phosphated_corrected_ancova_gad65_data <- fixed_gad65_data |>
  mutate(`glu corrected Enz Activity` = `Enz Activity` - Glu * glu_coef_ancova) |>
  mutate(`phosphate corrected Enz Activity` = `Enz Activity` - `pyridoxal phosphate conc.`
 * phosphate_coef_ancova) |>
   mutate(`Glu and phosphate corrected Enz Activity` = `Enz Activity` - `pyridoxal phosphate conc.` * phosphate_coef_ancova - Glu * glu_coef_ancova) |>
  pivot_longer(cols=c(`glu corrected Enz Activity`, `phosphate corrected Enz Activity`, `Glu and phosphate corrected Enz Activity`), values_to = "corrected Enz Activity")

```

```{r}
glutamate_and_phosphated_corrected_ancova_gad65_data |> ggplot(aes(x=`Enzyme Variant`, 
                               y=`Enz Activity`, 
                               color = `Enzyme Variant`)) +
  geom_violin() +
  geom_jitter(width = 0.25) +
