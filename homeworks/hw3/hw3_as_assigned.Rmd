...
**Answer: There is a significant difference in the enzyme activity of GAD65 enzyme variants caused by single nucleotide polymorphisms.**

This implies that the mutations introduced result in measurable changes in enzyme function, which could be linked to the enzyme's role in neurotransmitter regulation and, by extension, to psychiatric conditions. The observed differences in enzyme activity among variants suggest a potential functional impact of specific SNPs on GAD65, warranting further investigation into their role in the etiology of psychiatric disorders.**

### 1.B.2 Write out your statistical hypothesis (null hypothesis and alternate) and the formula you are fitting. (2 pt)

```{r}
str(fixed_gad65_data)
```

$$H_0: \mu_{A121C} = \mu_{A121F} = \mu_{A121L} = \mu_{A121R} = \mu_{A121T}$$
**H1: Not all means of enzyme activity is equal across different groups of enzyme variants.**
...
```{r}
fixed_gad65_data |> ggplot(aes(x=`Enzyme Variant`, y=`Enz Activity`)) +
  geom_violin() +
