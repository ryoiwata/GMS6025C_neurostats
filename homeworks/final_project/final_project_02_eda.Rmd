...
SUD_count_df <- cleaned_addiction_data |>
  pivot_longer(colnames(select(cleaned_addiction_data, starts_with("SUD_is"))), names_to = "SUD", values_to = "SUD_yn") |>
  filter(SUD_yn == "1") |>
...
religion_count_df <- cleaned_addiction_data |>
  pivot_longer(colnames(select(cleaned_addiction_data, starts_with("rel.is_"))), names_to = "religion", values_to = "religion_yn") |>
...
```{r}
SUD_counts <- colSums(select(cleaned_addiction_data, starts_with("SUD_is")) == 1)

counts_df <- as.data.frame(SUDg_counts) |>
  rownames_to_column() |>
  # filter(row) |>
  mutate(rowname = str_replace(rowname, "SUD_is_", "")) |>
  mutate(rowname = str_replace_all(rowname, "_", " "))

counts_df |>
  ggplot(aes(rowname, SUD_counts)) +
