...
```
```{r}
cleaned_addiction_data |> ggplot(aes(SUD.is_sum, color=gender, fill=gender)) +
...
```{r}
cleaned_addiction_data |> ggplot(aes(x=stressful_life.happened_to_you_and_witnessed_total, y=SUD.is_sum)) +
  geom_jitter() 
```
...
```{r}
cleaned_addiction_data |> ggplot(aes(x=aana_affiliation.tspe_positive, y=abstain_confidence_neg_mean_baseline, color=SUD.is_sum)) +
  geom_jitter()
```
...
```{r}
cleaned_addiction_data |> ggplot(aes(x=social_support.total, y=qol.mean_baseline, color=SUD.is_sum)) +
  geom_jitter()
```
