...
**TODO: figure out what line width is**
...
** TODO: figure out if each is one condition and what is departure delay?**
```{r}
str(flights)

unique(flights$carrier)

filter(flights, arr_delay > 2, dest%in%c("IAH", "HOU"), carrier%in%c("UA", "AA", "DL"), month%in%c(7,8,9), arr_delay>120, dep_delay<0, dep_delay )
...
** Answer: Yes, because there are 365 pairs of months and days for all flights that were in 2013. **
...
** It does not matter, because filtering and sorting are both maintained when followed by the other function **
...
filter(flights %>% arrange(arr_delay), arr_delay > 2)
...
** TODO: Figure out what is meant by select **
...
** Answer: the code selects for all columns that have the "TIME" string in the name regardless of the casing. Casing is ignored by default but can be changed to filter for it too by changing the `ignore.case` argument. **
...
```{r}
rename(flights, air_time_min = air_time) %>% select(air_time_min, everything())
...
ggplot(data = flights, aes(x = dep_time, y = dep_delay)) +
  geom_point()
...
** Answer: `count()` function counts the number of rows that have a given value in a specified column. Then it creates a new table with the counts of each group.
...
** Answer: The output will probably be a table that has a row for each group of rows from `df` that had a given `y` value. Then the mean of `x` was calculated for all rows in a given group. 

The pipeline outputs all of the `df` table into `group_by(y)` function, which creates a group of rows that had a given `y` value. These groupings are outputted to the   `summarize(mean_x = mean(x))` where the mean `x` was calculated for all rows in a given group. **
...
** Answer: The output will probably be a table that has a row for each group of rows from `df` that had a given `y` and `z` unique pair. Then the mean of `x` was calculated for all rows in a given group. 

The pipeline outputs all of the `df` table into `group_by(y, z)` function, which creates a group of rows that had a given `y` and `z` unique pair. These groupings are outputted to the   `summarize(mean_x = mean(x))` where the mean `x` was calculated for all rows in a given group.** 
...
** Answer: The output will probably be similar to the output from problem 6.d with a table that has a row for each group of rows from `df` that had a given `y` and `z` unique pair. Then the mean of `x` was calculated for all rows in a given group. 

The pipeline outputs all of the `df` table into `group_by(y, z)` function, which creates a group of rows that had a given `y` and `z` unique pair. These groupings are outputted to the   `summarize(mean_x = mean(x))` where the mean `x` was calculated for all rows in a given group.

The main difference is that the `group` functionality is removed in this output, and new tibble is created. ** 
**
...
Pipeline 1 was already discussed in question 6.d, so please reference that.

For Pipeline 2 output will probably be similar to the output from problem 6.d with a table that has a row for each group of rows from `df` that had a given `y` and `z` unique pair. Then the mean of `x` was calculated for all rows in a given group. The main difference is the usage of the `mutate` function which may indicate that a new dataframe is not created like for the `summarize` function. 

The pipeline outputs all of the `df` table into `group_by(y, z)` function, which creates a group of rows that had a given `y` and `z` unique pair. These groupings are outputted to the `mutate(mean_x = mean(x))` where the mean `x` was calculated for all rows in a given group. This information of the mean is placed in the original table with the mean of each group going to all the items in that group.** 
